<script lang="ts">
    ...
    const visibleProfiles = $derived(
        sortedProfiles.slice(0, MAX_VISIBLE_PROFILES),
    );
    const hiddenCount = $derived(
        Math.max(0, sortedProfiles.length - MAX_VISIBLE_PROFILES),
    );
</script>
...
<nav class="side-nav">
    ...
    <NavButton
        to="/library"
        isPrimary={(p) => p.url.pathname === "/library"}
        ...
        <div class="profile-shortcuts" aria-label="Profile Shortcuts">
            {#each visibleProfiles as profile (profile.id)}
                <NavButton
                    ...
                    isPrimary={(p) =>
                        p.url.pathname === `/library/${profile.id}`}
                >
                    <span class="profile-icon-shell">
                        <ProfileIcon
                            {profile}
                            class="rounded-full"
                            fallbackClass="h-5 w-5"
                        />
                    </span>
                </NavButton>
            {/each}
            ...
            {#if hiddenCount > 0}
                <NavButton
                    to="/library"
                    tooltip={`Show ${hiddenCount} more profiles`}
                >
