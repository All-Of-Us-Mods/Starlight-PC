<script lang="ts">
	...
	watch(
		...
		(isOpen) => {
			if (isOpen) {
				...
				unsubscribe = epicAuthService.subscribeToLoginEvents({
	...
	async function loginWithWebview() {
		try {
			await epicAuthService.loginWithWebview();
		} catch (e) {
	...
	async function loginWithCode() {
		...
		isLoading = true;
		try {
			await epicAuthService.login(authCode.trim());
			handleSuccess();
		} catch (e) {
			handleError(String(e));
		}
	...
	async function openAuthPage() {
		try {
			await openUrl(await epicAuthService.getAuthUrl());
		} catch (e) {
			showError(e);
		}
	}
