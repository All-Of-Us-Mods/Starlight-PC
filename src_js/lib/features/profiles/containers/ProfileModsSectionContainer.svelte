<script lang="ts">
  ...
  const modUpdatesQueryKey = $derived([
    "profile-mod-updates",
    profile.id,
    modUpdatesSignature,
  ]);
  const modUpdateStatuses = $derived(
    (modUpdatesQuery.data ?? {}) as ProfileModUpdatesMap,
  );
  ...
  const updatesAvailableCount = $derived(
    Object.values(modUpdateStatuses).filter((status) => status.isOutdated)
      .length,
  );
  const hasManagedModsForUpdates = $derived(managedModsForUpdates.length > 0);
  const isCheckingUpdates = $derived(
    hasManagedModsForUpdates &&
      (modUpdatesQuery.isPending || modUpdatesQuery.isFetching),
  );
  ...
  const deleteUnifiedMod = createMutation(() =>
    profileActions.deleteUnifiedMod(queryClient),
  );
  const cleanupMissingMods = createMutation(() =>
    profileActions.cleanupMissingMods(queryClient),
  );
  const installMods = createMutation(() =>
    profileActions.installMods(queryClient),
  );
  ...
  watch(
    ...
    (currentSignature) => {
      ...
      if (lastCleanupSignatureByProfile.get(profile.id) === currentSignature)
        return;
  ...
  async function handleDeleteMod() {
    ...
    deleteModDialogOpen = false;
    try {
      await deleteUnifiedMod.mutateAsync({
  ...
  function applyInstantUpdate(
    updatedMods: Array<{ modId: string; version: string }>,
  ) {
    const nextByModId = new Map(
      updatedMods.map((mod) => [mod.modId, mod.version]),
    );
    ...
    queryClient.setQueryData(
      profileUnifiedModsKey(profile.id),
      (current: UnifiedMod[] | undefined) => {
        if (!current) return current;
    ...
    queryClient.setQueryData(
      modUpdatesQueryKey,
      (current: ProfileModUpdatesMap | undefined) => {
        if (!current) return current;
        const next = { ...current };
  ...
  async function handleUpdateAll() {
    ...
    try {
      ...
      showSuccess(
        `Updated ${modsToUpdate.length} mod${modsToUpdate.length === 1 ? "" : "s"}`,
      );
      void modUpdatesQuery.refetch();
    } catch (error) {
...
<AlertDialog bind:open={deleteModDialogOpen}>
  <AlertDialogContent>
    <AlertDialogHeader>
      ...
      <AlertDialogDescription>
        Are you sure you want to remove this mod from the profile? The mod file
        will be deleted.
      </AlertDialogDescription>
    </AlertDialogHeader>
    <AlertDialogFooter>
