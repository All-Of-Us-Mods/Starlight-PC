...
#[cfg(windows)]
fn shortcut_icon_path(profile_id: &str) -> AppResult<PathBuf> {
    let icon_dir = crate::backend::directories::app_data_dir()?.join(SHORTCUT_ICON_DIR);
...
fn write_shortcut_icon(
    ...
    let icon_path = shortcut_icon_path(profile_id)?;
    resized
...
fn resolve_icon_path(
    ...
    match write_shortcut_icon(profile_id, icon_bytes) {
        Ok(path) => Ok(path),
        Err(error) => {
            log::warn!(
...
pub fn create_desktop_shortcut(
    ...
    #[cfg(windows)]
    {
        ...
        let desktop_dir = directories::UserDirs::new()
            ...
            .ok_or_else(|| AppError::state("Could not determine desktop directory".to_string()))?;
        ...
        let icon_path = resolve_icon_path(&profile.id, icon_bytes)?;
