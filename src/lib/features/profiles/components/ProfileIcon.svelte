...
<script lang="ts">
	...
	const customIconQuery = createQuery(() => profileQueries.binaryFile(customIconPath));
	const modIconQuery = createQuery(() => modQueries.byId(modIconId));
	...
	$effect(() => {
		...
		let cancelled = false;
		void buildCustomIconFilePath(profilePath, extension).then((resolvedPath) => {
			if (!cancelled) {
...
{#if iconSrc}
	<img src={iconSrc} {alt} class={cn('h-full w-full object-cover', className)} loading="lazy" />
{:else}
	<BoxIcon class={cn('text-muted-foreground/50', fallbackClass)} aria-hidden="true" />
{/if}
