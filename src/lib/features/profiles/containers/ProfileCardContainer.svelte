<script lang="ts">
  ...
  const retryBepInExInstall = createMutation(() =>
    profileActions.retryBepInExInstall(queryClient),
  );
  ...
  async function handleRemoveMod(mod: {
    id: string;
    source: "managed" | "custom";
  }) {
    try {
      const unifiedMod = findUnifiedModByChip(
        mod.id,
        mod.source,
        unifiedMods(),
      );
  ...
  const lastLaunched = $derived(
    profile.last_launched_at
      ? new Date(profile.last_launched_at).toLocaleDateString()
      : "Never",
  );
  ...
  const isInstalling = $derived(
    profile.bepinex_installed === false ||
      installState?.status === "installing",
  );
  const hasInstallError = $derived(installState?.status === "error");
  const isDisabled = $derived(isInstalling || isRunning);
  const isLaunchDisabled = $derived(
    isInstalling || (isRunning && !allowMultiInstanceLaunch),
  );
  const canLaunchAnother = $derived(
    isRunning && allowMultiInstanceLaunch && !isInstalling,
  );
  ...
  const totalPlayTime = $derived(
    (profile.total_play_time ?? 0) +
      (isRunning ? gameState.getSessionDuration(profile.id) : 0),
  );

  const modIds = $derived(profile.mods.map((m) => m.mod_id));
  const modsQueries = $derived(
    modIds.map((id) => createQuery(() => modQueries.info(id))),
  );
  ...
  const diskFilesQuery = createQuery(() =>
    profileQueries.diskFiles(profile.path),
  );
  ...
  const displayedMods = $derived(() =>
    showAllMods ? allMods : allMods.slice(0, 3),
  );
  const hiddenModCount = $derived(() => allMods.length - 3);

  const modCount = $derived(unifiedMods().length);

  function buildUnifiedMods(
    profileEntry: Profile,
    diskFiles: string[],
  ): UnifiedMod[] {
    const managedFiles = new Set(
      profileEntry.mods.map((mod) => mod.file).filter(Boolean),
    );
  ...
  function findUnifiedModByChip(
    ...
    return unifiedModEntries.find((mod) =>
      source === "managed"
        ? mod.source === "managed" && mod.mod_id === chipId
        : mod.file === chipId,
    );
  }
      ...
      <div class="min-w-0 flex-1 space-y-1.5">
        <div class="flex flex-wrap items-center gap-2">
          ...
          {#if hasInstallError}
            <Badge
              variant="outline"
              class="gap-1.5 border-destructive/50 text-destructive"
            >
