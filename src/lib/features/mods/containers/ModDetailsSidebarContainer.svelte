<script lang="ts">
  ...
  import InstallPanelContainer from "$lib/features/mods/containers/InstallPanelContainer.svelte";
  ...
  import {
    ...
    pickDefaultVersion,
    safeParseMarkdown,
  } from "$lib/features/mods/components/mod-utils";
  ...
  const deleteMod = createMutation(() =>
    profileActions.deleteUnifiedMod(queryClient),
  );
  ...
  const unifiedMods = $derived(() => {
    ...
    const managedFiles = new Set(
      profile.mods.map((m) => m.file).filter(Boolean),
    );
  ...
  const mod = $derived(modInfoQuery.data);
  ...
  const unifiedMod = $derived(
    profileId
      ? unifiedMods().find(
          (m: UnifiedMod) => m.source === "managed" && m.mod_id === modId,
        )
      : null,
  );
  ...
  <div class="scrollbar-styled min-h-0 flex-1 overflow-y-auto">
    ...
    {:else if modInfoQuery.isError}
      ...
      >
        ...
        <p class="text-sm text-muted-foreground">
          There was an error loading the mod details.
        </p>
      </div>
    {:else if mod}
      <div class="space-y-5 p-5">
        ...
        <div class="flex justify-center">
          <div
            class="relative h-44 w-44 overflow-hidden rounded-xl bg-muted ring-1 ring-border/50"
          >
        ...
        <div class="text-center">
          <h2 class="text-xl leading-tight font-bold tracking-tight">
            {mod.name}
          </h2>
        ...
        {#if mod?.links && mod.links.length > 0}
          <section class="space-y-2">
            ...
            <div class="flex flex-wrap gap-2">
              {#each mod.links as link, i (`${link.type}-${link.url}-${i}`)}
    ...
    {:else}
      ...
      >
        ...
        <p class="text-sm text-muted-foreground">
          The requested mod could not be loaded.
        </p>
      </div>
    {/if}
  </div>
