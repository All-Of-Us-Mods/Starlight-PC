<script lang="ts">
	...
	const resolvedDeps = $derived((resolvedDepsQuery.data ?? []) as ResolvedDependency[]);
	...
	const installModsMutation = createMutation(() => profileActions.installMods(queryClient));
