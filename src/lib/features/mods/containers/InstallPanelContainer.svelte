<script lang="ts">
  ...
  import type { ResolvedDependency } from "$lib/features/mods/components/types";
  ...
  const resolvedDeps = $derived(
    (resolvedDepsQuery.data ?? []) as ResolvedDependency[],
  );
  const installableDependencies = $derived(
    resolvedDeps.filter((d) => d.type !== "conflict"),
  );
  ...
  const installModsMutation = createMutation(() =>
  ...
  const installedModInProfile = $derived(
    selectedProfile?.mods.find((m) => m.mod_id === modId),
  );
  ...
  watch(
    ...
    (currentProfiles) => {
      ...
      if (currentProfiles.some((profile) => profile.id === selectedProfileId))
        return;
      ...
      const mostRecentlyLaunched = [...currentProfiles]
        ...
        .toSorted(
          (a, b) => (b.last_launched_at ?? 0) - (a.last_launched_at ?? 0),
        )[0];
      ...
      const nextProfileId =
        inferredProfileId ??
        mostRecentlyLaunched?.id ??
        mostRecentlyCreated?.id ??
        "";
      if (nextProfileId) {
  ...
  async function handleInstall() {
    ...
    try {
      ...
      const dependencyNameById = new Map(
        resolvedDeps.map((item) => [item.mod_id, item.modName]),
      );
      ...
      const installPlan = [
        ...
        ...resolvedDependencies
          .filter(
            (dependency) =>
              installedVersionByModId.get(dependency.mod_id) !==
          ...
          .map((dependency) => ({
            ...
            modName:
              dependencyNameById.get(dependency.mod_id) || dependency.modName,
          })),
      ];
          ...
          <div class="flex flex-col gap-0.5">
            ...
            {#if installedVersion !== selectedVersion}
              <span class="text-xs opacity-80"
                >Installing will replace with v{selectedVersion}</span
              >
            {/if}
          </div>
        </div>
              ...
              >
                <div class="flex items-center gap-2.5">
                  <Switch
                    ...
                    onCheckedChange={() =>
                      toggleDependency(dep.mod_id, dep.type)}
                    disabled={isInstalling ||
                      isInstalled ||
                      dep.type === "required"}
                    class="scale-90"
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-center justify-between text-xs">
                      ...
                      <span class="text-muted-foreground">
                        {gameState.getModDownloadStageText(
